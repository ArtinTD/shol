...
public class Consumer implements Runnable{
    ...
    public Consumer(String groupId, String topic, Properties props, Storage storage) {
    ...
    public void run() {
        ...
        while (!Thread.currentThread().isInterrupted()) {
            ...
            for (ConsumerRecord<String, String> record : records) {
                try {
                    if (!storage.exists(record.value())) {
                        consumingQueue.add(record.value());
                    }
                } catch (IOException e) {
                    logger.fatal("Can't check existence from HBase");
                }
            }
            synchronized (LOCK) {
                try {
